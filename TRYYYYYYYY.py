...
data_all = [australia_df,austria_df, brazil_df, germany_df, usa_df, russia_df]   #It WORKS TRYING TO JOIN INDICATOR EXCEL 
...
# all_data = [gdp_df,inflation_df, unemployment_df]   #trying this from the uppe r
...
if select == "Line Graph":
    ...
    line_g = st.sidebar.radio('Indicator', ('GDP', 'Infaltion Rate', 'Unemployment Rate'))
    if line_g == "GDP":
        ...
        if line_g == "GDP" and len(choice) > 0: 
            result_df=df_all[(df_all.Country.isin(choice)) & (df_all["Year"]>=year[0]) & (df_all["Year"]<=year[1])][[line_g,"Year","Country"]]
            fig = px.line(result_df, x='Year', y=line_g[0], color="Country", height = 500)
            st.plotly_chart(fig)
...
#     if len(indicator_choice)>0 and len(choice) > 0: 
#         result_df=all_df[(all_df.Australia.Austria.Brazil.Germany.Russia.USA.isin(choice)) & (all_df["Year"]>=year[0]) & (all_df["Year"]<=year[1])][[indicator_choice[0],"Year","Australia","Austria","Brazil","Germany","Russia","USA"]]

#         fig = px.line(result_df, x='Year', y=indicator_choice[0], color=["Australia","Austria","Brazil","Germany","Russia","USA"], height = 500)
#         st.plotly_chart(fig)
...
#ПОПЫТКА , а сверху попытка ос объединением индикаторов"
...
#             result_df=all_df[(all_df.Country.isin(choice)) & (all_df["Year"]>=year[0]) & (all_df["Year"]<=year[1])][[lg_indicator,"Year","Country"]]
...
all_data = [gdp_df,inflation_df, unemployment_df]
all_df= pd.concat(all_data,ignore_index=True)